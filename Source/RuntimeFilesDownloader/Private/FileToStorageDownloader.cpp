...
void UFileToStorageDownloader::DownloadFileToStorage(const FString& URL, const FString& SavePath, float Timeout, const FString& ContentType, bool bForceByPayload)
{
	if (URL.IsEmpty())
	{
		...
		OnDownloadComplete.ExecuteIfBound(EDownloadToStorageResult::InvalidURL, SavePath);
		RemoveFromRoot();
		return;
	}
	...
	if (SavePath.IsEmpty())
	{
		...
		OnDownloadComplete.ExecuteIfBound(EDownloadToStorageResult::InvalidSavePath, SavePath);
		RemoveFromRoot();
		return;
	}
